...
public class SingleTreeNode {
    ...
    protected MCTSEAHParams params;
    ...
    public static SingleTreeNode createRootNode(MCTSEAHPlayer player, AbstractGameState state, Random rnd, Supplier<? extends SingleTreeNode> factory) {
    ...
    protected void instantiate(SingleTreeNode parent, AbstractAction actionToReach, AbstractGameState state) {
        ...
        if (params.information != MCTSEAHEnums.Information.Closed_Loop && (params.maintainMasterState || depth == 0)) {
...
//                actionsFromOpenLoopState.stream().map(a -> "\t" + a.toString() + "\n").collect(joining()));
    ...
    private int validVisitsFor(AbstractAction action) {
        if (params.information == MCTSEAHEnums.Information.Closed_Loop)
            return nVisits;
        ActionStats stats = actionValues.get(action);
    ...
    protected AbstractAction treePolicyAction(boolean explore) {
        if (params.opponentTreePolicy == MCTSEAHEnums.OpponentTreePolicy.SelfOnly && parent != null && openLoopState != null && openLoopState.getCurrentPlayer() != decisionPlayer)
        ...
        if (availableActions.size() == 1) {
            ...
            actionChosen = switch (params.treePolicy) {
                ...
                case RegretMatching, EXP3 -> {
                    ...
                    if (this == root && params.treePolicy == MCTSEAHEnums.TreePolicy.RegretMatching && nVisits > actionValues.length && nVisits % Math.min(actionValues.length, 10) == 1) {
...
//            }
    ...
    private double ucbValue(AbstractAction action) {
        ...
        // consider OMA term
        if (params.omaVisits > 0 && (params.opponentTreePolicy == MCTSEAHEnums.OpponentTreePolicy.OMA_All || params.opponentTreePolicy == MCTSEAHEnums.OpponentTreePolicy.OMA)) {
    ...
    protected double[] rollout(int lastActor) {
        ...
        if (params.rolloutLength > 0 || params.rolloutTermination != MCTSEAHEnums.RolloutTermination.DEFAULT) {
    ...
    protected void normaliseRewardsAfterIteration(double[] result) {
        ...
        if (params.normaliseRewards || params.treePolicy == MCTSEAHEnums.TreePolicy.UCB_Tuned) {
    ...
    protected double[] backUpSingleNode(AbstractAction actionTaken, double[] result) {
        ...
        if (!actionsToConsider.contains(actionTaken)) {
            if (params.opponentTreePolicy != MCTSEAHEnums.OpponentTreePolicy.MCGS && params.opponentTreePolicy != MCTSEAHEnums.OpponentTreePolicy.MCGSSelfOnly)
    ...
    public AbstractAction bestAction() {
        ...
        MCTSEAHEnums.SelectionPolicy policy = params.selectionPolicy;
        // check to see if all nodes have the same number of visits
        // if they do, then we use average score instead
        if (params.selectionPolicy == MCTSEAHEnums.SelectionPolicy.ROBUST &&
                Arrays.stream(actionVisits()).boxed().collect(toSet()).size() == 1) {
            policy = MCTSEAHEnums.SelectionPolicy.SIMPLE;
        }
        if (params.selectionPolicy == MCTSEAHEnums.SelectionPolicy.TREE || params.treePolicy == MCTSEAHEnums.TreePolicy.EXP3) {
            // EXP3 uses the tree policy (without exploration)
            bestAction = treePolicyAction(false);
        } else if (params.treePolicy == MCTSEAHEnums.TreePolicy.RegretMatching && !regretMatchingAverage.isEmpty()) {
            ...
            for (AbstractAction action : forwardModel.computeAvailableActions(state, params.actionSpace)) {
                ...
                if (actionValues.get(action) != null) {
                    ...
                    if (policy == MCTSEAHEnums.SelectionPolicy.SIMPLE)
...
//                System.out.println("Only one visit to root node - insufficient information - hopefully due to JVM warming up");
    ...
    public String toString() {
        ...
        for (AbstractAction action : sortedActions) {
            ...
            if (params.opponentTreePolicy == MCTSEAHEnums.OpponentTreePolicy.OneTree) {
                valueString = IntStream.range(0, openLoopState.getNPlayers())
